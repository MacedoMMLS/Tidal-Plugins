...
export const makeTags = async (extTrackItem: ExtendedMediaItem): Promise<MetaTags> => {
	...
	if (tidalAlbum !== undefined) {
		...
		if (tidalAlbum.releaseYear) {
			tags.year = tidalAlbum.releaseYear.toString();
		}

		if (!tags.year && releaseAlbum?.date) {
			const mbDate = new Date(releaseAlbum.date);
			if (!isNaN(mbDate.getTime())) {
				tags.year = mbDate.getFullYear().toString();

				if (!tags.date) {
					const month = String(mbDate.getMonth() + 1).padStart(2, "0");
					const day = String(mbDate.getDate()).padStart(2, "0");
					tags.date = `${tags.year}-${month}-${day}`;
				}
			}
		}

		if (!tags.year && tidalTrack.streamStartDate) {
			const streamDate = new Date(tidalTrack.streamStartDate);
			if (!isNaN(streamDate.getTime())) {
				tags.year = streamDate.getFullYear().toString();

				if (!tags.date) {
					const month = String(streamDate.getMonth() + 1).padStart(2, "0");
					const day = String(streamDate.getDate()).padStart(2, "0");
					tags.date = `${tags.year}-${month}-${day}`;
				}
			}
		}
